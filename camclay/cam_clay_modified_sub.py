...
def loading(carry: MidState, _, parameters: SoilParameters):
    ...
    _pn1 = _hydrostatic * jnp.exp(jnp.sqrt(3) * _deviatoric / parameters.ymf / _hydrostatic)
    _pn1e = 98 * jnp.exp(carry.plastic_v / parameters.hee)
    _ddd = jnp.maximum((1 + parameters.void_rate)*parameters.hee*jnp.log(_pn1e/_pn1), 0)
    _hdd = ((1 - jnp.sqrt(3) * _deviatoric / parameters.ymf / _hydrostatic)
            + parameters.ocr_dissipation*(_ddd**2)) / _hydrostatic
    ...
    new_carry = MidState(stiffness=stiffness, stress=_stress, strain=_strain, plastic_v=_plastic_v)
...
def initialize(parameters: SoilParameters) -> MidState:
    ...
    _stiffness = _stiffness + jnp.eye(6) * _dmu
    _plastic_v = parameters.hee * jnp.log(parameters.overconsolidation*parameters.sigma0/98)
    return MidState(stress=_stress, stiffness=_stiffness, strain=_strain, plastic_v=_plastic_v)
